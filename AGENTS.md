...
- **Current version:** `v0.7.5-dev`
- **Reason:** v0.7.0 is implemented as a verified integrated 3D workbench, and the v0.7.1-v0.7.5 development foundation is now active: theme manager and three QSS themes, View-menu theme switching, SVG icon set, transparent viewport HUD, ROS-style `package://` mesh resolution, URDF material/alpha preservation, OpenGL opacity path, PyInstaller onefile/onedir build pipeline, exe metadata/icon, package smoke test, onboarding scaffold, crash logs, update-version helper, dev signing script, and 3D reliability regression coverage.
...
The safest next step after `v0.7.5-dev` is to finish release-grade validation: clean Win11 exe verification, release certificate signing, icon/HUD visual QA on a real display, large-scene picking performance, part visibility/opacity UI, and v0.7.6 performance profiling.
